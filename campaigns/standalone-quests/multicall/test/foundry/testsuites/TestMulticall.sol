...
contract TestMulticall is Test {
    ...
    constructor(address _callee, address _referenceCallee, string memory _testDataKey) {
        ...
        for (uint256 i = 0; i < readCalls.length; i++) {
            (bool success, bytes memory data) = referenceCallee.call(readCalls[i]);
    ...
    function test_batch_transactions_in_multicall() external {
        ...
        for (uint256 i = 0; i < readCalls.length; i++) {
            (bool success, bytes memory actual) = callee.staticcall(readCalls[i]);
        ...
        assertTrue(returnData.length == expectedReturns.length, "Unexpected number of return values");

        for (uint256 i = 0; i < writeCalls.length; i++) {
            assertEq(returnData[i], expectedReturns[i], "Unexpected return value forwarded");
        }
    }
